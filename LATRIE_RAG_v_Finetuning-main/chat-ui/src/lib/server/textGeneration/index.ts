...
import { runMyRag } from "../my_rag/runMyRag";
...
async function* textGenerationWithoutTitle(
	...
	const { model, conv, messages, assistant, isContinue, webSearch, myRag, toolsPreference } = ctx;
	...
	let myRagResult: MyRag | undefined;

	if (!isContinue && !model.tools && myRag && !conv.assistantId) {
		myRagResult = yield* runMyRag(conv, messages);
	}
	...
	const processedMessages = await preprocessMessages(messages, webSearchResult, myRagResult, convId);
