<script lang="ts">
	...
	import MyRag from "../MyRag.svelte"
	...
	import UploadBtnRag from "../UploadBtnRag.svelte";
	...
	import UploadedFileRag from "./UploadedFileRag.svelte";
	import { useSettingsStore } from "$lib/stores/settings";
	import type { ToolFront } from "$lib/types/Tool";
	import { myRagParameters } from "$lib/stores/myRagParameters.js";
			...
			<div class="flex w-full pb-3">
				{#if !assistant}
					...
					{:else if $page.data.settings?.searchEnabled}
						<WebSearchToggle />
					{:else if $page.data.settings?.RagEnabled}
						<MyRag />
						{#if $myRagParameters.useRag}
							<UploadBtnRag bind:files mimeTypes={activeMimeTypes} classNames="ml-auto"/>
						{/if}
					{/if}
				{/if}
				{#if loading}
