...
def plot_metrics(data, situation, title):
    ...
    plot_data = []
    for entry in data:
        embedding = entry["parameters"]["embedding model"]
    ...
    g1.map_dataframe(sns.lineplot, x='embedding model', y='BLEU', label='BLEU', marker='o', color='blue')
    g1.map_dataframe(sns.lineplot, x='embedding model', y='ROUGE', label='ROUGE', marker='o', color='red')
    g1.map_dataframe(sns.lineplot, x='embedding model', y='METEOR', label='METEOR', marker='o', color='green')
    g1.map_dataframe(sns.lineplot, x='embedding model', y='BERTScore', label='BERTScore', marker='o', color='purple')

    g1.set_axis_labels('Embedding models', 'Scores')
    ...
    g2.map_dataframe(sns.lineplot, x='embedding model', y='BertScore precision', label='BertScore precision', marker='o', color='blue')
    g2.map_dataframe(sns.lineplot, x='embedding model', y='BertScore recall', label='BertScore recall', marker='o', color='red')
    g2.map_dataframe(sns.lineplot, x='embedding model', y='BertScore F1', label='BertScore F1', marker='o', color='green')

    g2.set_axis_labels('Embedding models', 'BERTScores')
...
def add_constant_parameters_page(pdf, data):
    ...
    del constant_parameters["embedding model"]
...
if __name__ == "__main__":
    ...
    json_file = f"../json_result_files/embedding/results_embedding_config_query.json"
    ...
    if os.path.isfile(os.path.join("../json_result_files", json_file)):
        ...
        pdf_filename = "pdf_files/embedding/embedding_graphs.pdf"
    ...
    json_file = "../json_result_files/embedding/results_embedding_config_queries.json"

    if os.path.isfile(os.path.join("../json_result_files", json_file)):
        with open(json_file, 'r') as f:
            data = json.load(f)

        pdf_filename = "pdf_files/embedding/embedding_graphs_queries.pdf"
