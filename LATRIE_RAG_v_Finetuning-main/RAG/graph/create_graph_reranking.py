...
def plot_metrics(data, situation, title):
    ...
    plot_data = []
    for entry in data:
        top_k = entry["parameters"]["top_k"]
        reranking = entry["parameters"]["re-ranking"]
    ...
    g1 = sns.FacetGrid(df, col='top_k', height=4, aspect=1.5)
    g1.map_dataframe(sns.lineplot, x='re-ranking', y='BLEU', label='BLEU', marker='o', color='blue')
    g1.map_dataframe(sns.lineplot, x='re-ranking', y='ROUGE', label='ROUGE', marker='o', color='red')
    g1.map_dataframe(sns.lineplot, x='re-ranking', y='METEOR', label='METEOR', marker='o', color='green')
    g1.map_dataframe(sns.lineplot, x='re-ranking', y='BERTScore', label='BERTScore', marker='o', color='purple')

    g1.set_axis_labels('Re-ranking', 'Scores')
    g1.set_titles('Top_k = {col_name}')
    ...
    # Utilisation de seaborn pour créer des faceted plots
    g2 = sns.FacetGrid(df, col='top_k', height=4, aspect=1.5)
    g2.map_dataframe(sns.lineplot, x='re-ranking', y='BertScore precision', label='BertScore precision', marker='o', color='blue')
    g2.map_dataframe(sns.lineplot, x='re-ranking', y='BertScore recall', label='BertScore recall', marker='o', color='red')
    g2.map_dataframe(sns.lineplot, x='re-ranking', y='BertScore F1', label='BertScore F1', marker='o', color='green')

    g2.set_axis_labels('Re-ranking', 'BERTScores')
    g2.set_titles('Top_k = {col_name}')
    ...
    for ax in g2.axes.flat:
        handles, labels = ax.get_legend_handles_labels()
        if handles:
            ax.legend(handles[:3], ['BertScore precision', 'BertScore recall', 'BertScore F1'], loc='best')

    return g1, g2

def add_constant_parameters_page(pdf, data):
    constant_parameters = data[0]["parameters"].copy()
    del constant_parameters["re-ranking"]
    del constant_parameters["top_k"]
...
if __name__ == "__main__":
    ...
    json_file = f"../json_result_files/reranking/results_reranking_config_query.json"
    ...
    if os.path.isfile(os.path.join("../json_result_files", json_file)):
        ...
        pdf_filename = "pdf_files/reranking/reranking_graphs.pdf"
    ...
    json_file = "../json_result_files/reranking/results_reranking_config_queries.json"

    if os.path.isfile(os.path.join("../json_result_files", json_file)):
        with open(json_file, 'r') as f:
            data = json.load(f)

        pdf_filename = "pdf_files/reranking/reranking_graphs_queries.pdf"
