...
def plot_metrics(data, situation, title):
    param_values = [entry["parameters"]["chunk size"] for entry in data]
    
    # Extract metrics for the first plot
    ...
    # Extract BERTScore precision, recall, and F1 for the second plot
...
def add_constant_parameters_page(pdf, data):
    ...
    del constant_parameters["chunk size"]
...
if __name__ == "__main__":
    ...
    json_file = "../json_result_files/chunk_size/results_chunksize_config_query.json" 
    ...
    if os.path.isfile(os.path.join("../json_result_files", json_file)):
        ...
        pdf_filename = "pdf_files/chunk_size/chunk_size_graphs.pdf"
    ...
    json_file = "../json_result_files/chunk_size/results_chunksize_config_queries.json"

    if os.path.isfile(os.path.join("../json_result_files", json_file)):
        with open(json_file, 'r') as f:
            data = json.load(f)

        pdf_filename = "pdf_files/chunk_size/chunk_size_graphs_queries.pdf"
